<template>
    <div class="my-activity-container">
      <template v-if="review.length!=0">
        <my-activity v-for="(item,index) in review" :content="item" :key="index" :isActivity="true" @refresh="refresh"></my-activity>
      </template>
      <template v-else>
...
export default {
  ...
  methods: {
    ...
    getActivity() {
      var self = this;
      self.clear();
      const address = `${self.url}/api/Apply/${self.user.Id}/applyReview`;
      axios
        .get(address)
        .then(res => {
